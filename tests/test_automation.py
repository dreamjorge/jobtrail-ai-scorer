...
class FakeJobTrail:
    ...
    def search(self, payload):
        ...
        return [{"site": "indeed", "id": "source-1", "title": "Python Engineer", "company": "Acme", "description": "good", "job_url": "https://jobs.test/1", "location": "Queretaro", "is_remote": False}]
...
class FakeScorer:
    ...
    def __call__(self, job_id, config_path):
        ...
        self.jobs[job_id]["notes"] = [{"body": '[AI_JOB_SCORE_V1]\n' + json.dumps({"score": 91, "recommendation": "PRIORITY_APPLY", "strengths": ["Python"], "gaps": ["None"]})}]
...
def test_search_payloads_split_sites_and_locations():
    ...
    assert payloads == [
        {"sites": ["linkedin", "indeed"], "searchTerm": config.search_terms, "location": "Queretaro", "resultsWanted": 10, "hoursOld": 72, "isRemote": False},
        {"sites": ["linkedin", "indeed"], "searchTerm": config.search_terms, "location": "remote", "resultsWanted": 10, "hoursOld": 72, "isRemote": True},
    ]


def test_import_mapping_omits_nulls_and_maps_jobspy_fields():
    assert map_jobspy_job({"site": "indeed", "id": "x", "company": "C", "title": "T", "description": "D", "job_url": "U", "location": "Q", "is_remote": True, "min_amount": 10, "max_amount": 20, "currency": "USD", "job_type": "fulltime"}) == {
        "source": "indeed", "sourceJobId": "x", "company": "C", "position": "T", "description": "D", "jobUrl": "U", "location": "Q", "remote": True, "salaryMin": 10, "salaryMax": 20, "salaryCurrency": "USD", "jobType": "fulltime"
    }


def test_import_mapping_omits_none_jobspy_fields():
    assert map_jobspy_job({"site": "indeed", "id": "x", "is_remote": None, "min_amount": None, "max_amount": None, "currency": None, "job_type": None}) == {
...
def test_http_client_uses_discover_routes_for_search_and_import():
    ...
    assert recorder.posts == [("/api/discover/search", payload), ("/api/discover/import", payload)]
...
def test_run_scores_real_mode_and_notifies_once_for_best_match():
    ...
    result = JobSearchAutomation(gateway, scorer=scorer, notifier=notifier).run(config=AutomationConfig(scorer_config_path="safe/config.yaml", notify_enabled=True))
...
def test_selected_notification_contains_job_and_score_data(monkeypatch):
    ...
    JobSearchAutomation(gateway, scorer=scorer, notifier=notifier).run(
        config=AutomationConfig(scorer_config_path="safe/config.yaml", notify_enabled=True)
    )
...
def test_no_notification_when_score_below_threshold():
    ...
    notifier = FakeNotifier()
    result = JobSearchAutomation(gateway, scorer=score, notifier=notifier).run(config=AutomationConfig(scorer_config_path="safe/config.yaml", notify_enabled=True))
    assert result.selected is None
    assert notifier.messages == []


def test_parse_score_note_is_safe_and_uses_latest_valid_marker():
    notes = [{"body": "[AI_JOB_SCORE_V1]\nnot-json"}, {"body": '[AI_JOB_SCORE_V1]\n{"score":88,"recommendation":"APPLY","strengths":["a"],"gaps":["b"]}'}]
...
def test_default_scorer_preserves_direct_cli_job_id_invocation(monkeypatch):
    ...
    result = JobSearchAutomation(gateway).run(
        config=AutomationConfig(scorer_config_path="safe/config.yaml", scorer_command="jobtrail-ai-scorer")
    )

    assert calls == [(["jobtrail-ai-scorer", "score", "--config", "safe/config.yaml", "--job-id", "j1", "--force"], True)]
...
def test_notification_summary_allowlist_and_bounded_text():
    ...
    assert set(summary) == {"title", "company", "location", "score", "recommendation", "strengths", "gaps", "jobUrl"}
